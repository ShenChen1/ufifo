...
static int __ufifo_bsem_deinit(sem_t *bsem) { return sem_destroy(bsem); }
...
static int __ufifo_init_from_shm(ufifo_t *ufifo) {
  ...
  ufifo->ctrl = mmap(NULL, ufifo->ctrl_size, (PROT_READ | PROT_WRITE),
                     MAP_SHARED, ufifo->ctrl_fd, 0);
  ...
  ufifo->shm_mem = mmap(NULL, ufifo->shm_size, (PROT_READ | PROT_WRITE),
                        MAP_SHARED, ufifo->shm_fd, 0);
...
static unsigned int __ufifo_peek_len(ufifo_t *handle, unsigned int offset) {
  ...
  if (len && handle->hook.recsize) {
    ...
    len =
        handle->hook.recsize(handle->shm_mem + offset,
                             *handle->kfifo.mask - offset + 1, handle->shm_mem);
  }
...
static unsigned int __ufifo_peek_tag(ufifo_t *handle, unsigned int offset) {
  ...
  if (handle->hook.rectag) {
    offset &= *handle->kfifo.mask;
    ret =
        handle->hook.rectag(handle->shm_mem + offset,
                            *handle->kfifo.mask - offset + 1, handle->shm_mem);
  }
...
static unsigned int __ufifo_put(ufifo_t *handle, void *buf, unsigned int size,
                                long millisec) {
  int ret;
  unsigned int len;
  ...
  if (handle->hook.recput) {
    ...
    len = handle->hook.recput(handle->shm_mem + len,
                              *handle->kfifo.mask - len + 1, handle->shm_mem,
                              buf);
    assert(size == len);
    smp_wmb();
    *handle->kfifo.in += len;
...
unsigned int ufifo_put_timeout(ufifo_t *handle, void *buf, unsigned int size,
                               long millisec) {
  UFIFO_CHECK_HANDLE_FUNC(handle);
  return __ufifo_put(handle, buf, size, millisec);
}

static unsigned int __ufifo_get(ufifo_t *handle, void *buf, unsigned int size,
                                long millisec) {
  int ret;
  unsigned int len;
  ...
  if (handle->hook.recget) {
    ...
    len = handle->hook.recget(handle->shm_mem + len,
                              *handle->kfifo.mask - len + 1, handle->shm_mem,
                              buf);
    smp_wmb();
...
unsigned int ufifo_get_timeout(ufifo_t *handle, void *buf, unsigned int size,
                               long millisec) {
  UFIFO_CHECK_HANDLE_FUNC(handle);
  return __ufifo_get(handle, buf, size, millisec);
}

static unsigned int __ufifo_peek(ufifo_t *handle, void *buf, unsigned int size,
                                 long millisec) {
  int ret;
  unsigned int len;
...
unsigned int ufifo_peek_timeout(ufifo_t *handle, void *buf, unsigned int size,
                                long millisec) {
  UFIFO_CHECK_HANDLE_FUNC(handle);
