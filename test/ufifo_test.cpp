...
// =============================================================================
// Missing tests: Log Interface, Timeout/Blocking APIs, Errors, Broker IPC
// =============================================================================

static int g_log_called = 0;
static void custom_log_handler(void *arg, const char *fmt, va_list ap) {
    (void)arg;
    (void)fmt;
    (void)ap;
    g_log_called++;
...
TEST(UfifoLogTest, SetLogHandler) {
    g_log_called = 0;
    ufifo_set_log_handler(custom_log_handler, nullptr);
    ...
    ufifo_t *fifo = nullptr;
    ASSERT_EQ(0, ufifo_open("log_test_fifo", &init, &fifo));
    ASSERT_NE(fifo, nullptr);

    // ufifo_dump uses the log handler
    ufifo_dump(fifo);
    ASSERT_GT(g_log_called, 0);
...
TEST(UfifoTimeoutTest, TimeoutApis) {
    ...
    init.alloc.max_users = 1;
    ufifo_t *fifo = nullptr;
    ASSERT_EQ(0, ufifo_open("timeout_test_fifo", &init, &fifo));
    ASSERT_NE(fifo, nullptr);

    // put timeout on empty fifo should succeed immediately
    char buf[10] = "abc";
    EXPECT_EQ(3, ufifo_put_timeout(fifo, buf, 3, 100));

    // fill fifo
    char fill_buf[64] = {0};
    ufifo_put(fifo, fill_buf, 61); // total 64 bytes

    // put timeout on full fifo should timeout and return 0
    EXPECT_EQ(0, ufifo_put_timeout(fifo, buf, 3, 50));

    // get timeout should succeed
    char out_buf[10];
    ...
    // empty the fifo
    char empty_buf[64];
    ufifo_get(fifo, empty_buf, 61);

    // get timeout on empty fifo should timeout and return 0
    EXPECT_EQ(0, ufifo_get_timeout(fifo, out_buf, 3, 50));

    // peek timeout on empty fifo should timeout and return 0
    EXPECT_EQ(0, ufifo_peek_timeout(fifo, out_buf, 3, 50));
...
TEST(UfifoBlockTest, BlockApis) {
    ...
    init.alloc.max_users = 1;
    ufifo_t *fifo = nullptr;
    ASSERT_EQ(0, ufifo_open("block_test_fifo", &init, &fifo));
    ASSERT_NE(fifo, nullptr);

    // Test block put on empty/partial
    char buf[10] = "abc";
    EXPECT_EQ(3, ufifo_put_block(fifo, buf, 3));

    // Test block get
    char out_buf[10];
    EXPECT_EQ(3, ufifo_get_block(fifo, out_buf, 3));

    // Test block peek
    ufifo_put(fifo, buf, 3);
    EXPECT_EQ(3, ufifo_peek_block(fifo, out_buf, 3));
...
TEST(ErrorTest, MmapAndShmOpenFail) {
    ...
    // Use an excessively long name to trigger shm_open / param failures
    std::string long_name(5000, 'x');
    ufifo_t *fifo = nullptr;
    int ret = ufifo_open(long_name.c_str(), &init, &fifo);
    EXPECT_NE(ret, 0);
...
extern "C" {
    ...
    void __ufifo_broker_daemon(void *ctx);
}
...
TEST(BrokerInternalTest, SendFds) {
    ...
    // Test normal send
    EXPECT_EQ(0, __ufifo_send_fds(sv[1], &fd_to_send, 1));
...
TEST(BrokerInternalTest, DaemonExitOnShmUnlink) {
    ...
    ufifo_t *fifo = nullptr;
    ASSERT_EQ(0, ufifo_open("daemon_exit_test", &init, &fifo));
    ASSERT_NE(fifo, nullptr);
    ...
    if (pid == 0) {
        ...
        if (0 == ufifo_open("daemon_exit_test", &init_attach, &child_fifo)) {
    ...
    // Now destroy the fifo. The broker daemon should notice the shm file is gone during its poll and exit.
    ufifo_destroy(fifo);

    // Wait a bit for the broker daemon to poll and exit (BROKER_POLL_INTERVAL_MS is 2000ms)
    usleep(2500000); // 2.5s to ensure the poll loop runs and exits
