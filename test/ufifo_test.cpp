...
class UfifoTestBase : public ::testing::Test {
  protected:
    ...
    std::string GenerateName(const char *prefix)
    {
        ...
        fifo_name_ = std::string(prefix) + "_" + std::to_string(counter++) + "_" + std::to_string(getpid());
        return fifo_name_;
    }
};
...
TEST_F(ByteSoleMPSC, Boundary)
{
    ...
    for (int p = 0; p < 4; p++) {
        producers.emplace_back([&]() {
            int val = 1;
            if (ufifo_put(fifo_, &val, sizeof(val)) > 0)
                success++;
        });
    }
    for (auto &t : producers)
...
TEST_F(ByteSoleMPMC, Boundary)
{
    ...
    // drain
    int out;
    while (ufifo_get(fifo_, &out, sizeof(out)) > 0)
        consumed++;
...
TEST_F(ByteSharedSingle, FifoFullEmpty)
{
    ...
    while (ufifo_put(fifos_[0], &val, sizeof(val)) > 0)
        count++;
    EXPECT_GT(count, 0u);
...
TEST_F(ByteSharedSPMC, Broadcast)
{
    ...
    for (auto &c : consumers) {
        int count = 0;
        int out;
        while (ufifo_get(c, &out, sizeof(out)) > 0)
            count++;
        EXPECT_EQ(N, count);
    }
}
...
TEST_F(ByteSharedSPMC, ConsumerIndependence)
{
    ...
    int c2_count = 0;
    int out;
    while (ufifo_get(c2, &out, sizeof(out)) > 0)
        c2_count++;
    EXPECT_EQ(10, c2_count);

    // c1 should still have 5 remaining
    int c1_count = 0;
    while (ufifo_get(c1, &out, sizeof(out)) > 0)
        c1_count++;
...
TEST_F(ByteSharedSPMC, Stress)
{
    ...
    for (auto &c : consumers) {
        consumer_threads.emplace_back([&, c]() {
            ...
            while (count < N) {
                int out;
                if (ufifo_get(c, &out, sizeof(out)) > 0)
                    count++;
                else
                    std::this_thread::yield();
            }
...
TEST_F(ByteSharedMPSC, Boundary)
{
    ...
    for (int p = 0; p < 4; p++) {
        producers.emplace_back([&]() {
            int val = 1;
            if (ufifo_put(fifos_[0], &val, sizeof(val)) > 0)
                success++;
        });
    }
    for (auto &t : producers)
...
TEST_F(ByteSharedMPMC, Boundary)
{
    ...
    for (auto &t : producers)
        t.join();
    int consumed = 0, out;
    while (ufifo_get(consumer, &out, sizeof(out)) > 0)
        consumed++;
...
TEST_F(RecSoleSingle, SkipRecord)
{
    ...
    // Put 2 records
    rec->size = 3;
    memcpy(rec->data, "aaa", 3);
    ufifo_put(fifo_, rec, sizeof(TestRecord) + rec->size);
    rec->size = 3;
    memcpy(rec->data, "bbb", 3);
    ufifo_put(fifo_, rec, sizeof(TestRecord) + rec->size);
...
TEST_F(RecSoleSingle, FifoFullEmpty)
{
    ...
    while (true) {
        rec->size = 8;
        memset(rec->data, 'x', 8);
        if (ufifo_put(fifo_, rec, sizeof(TestRecord) + rec->size) == 0)
            break;
        count++;
    }
    EXPECT_GT(count, 0);
...
TEST_F(RecSoleSPSC, Basic)
{
    ...
    while (count < N) {
        char out_buf[32];
        if (ufifo_get(fifo_, out_buf, sizeof(out_buf)) > 0)
            count++;
        else
            std::this_thread::yield();
    }
...
TEST_F(RecSoleMPMC, Boundary)
{
    ...
    int consumed = 0;
    char out_buf[32];
    while (ufifo_get(fifo_, out_buf, sizeof(out_buf)) > 0)
        consumed++;
    EXPECT_EQ(produced.load(), consumed);
}
...
TEST_F(RecSharedSingle, FifoFullEmpty)
{
    ...
    while (true) {
        rec->size = 8;
        if (ufifo_put(fifos_[0], rec, sizeof(TestRecord) + rec->size) == 0)
            break;
        count++;
    }
    EXPECT_GT(count, 0);
...
TEST_F(RecSharedSPSC, Basic)
{
    ...
    while (count < N) {
        char out_buf[32];
        if (ufifo_get(consumer, out_buf, sizeof(out_buf)) > 0)
            count++;
        else
            std::this_thread::yield();
    }
...
TEST_F(RecSharedSPMC, Broadcast)
{
    ...
    for (auto &c : consumers) {
        int count = 0;
        char out_buf[32];
        while (ufifo_get(c, out_buf, sizeof(out_buf)) > 0)
            count++;
        EXPECT_EQ(N, count);
    }
}
...
TEST_F(RecSharedSPMC, ConsumerIndependence)
{
    ...
    int c2_count = 0;
    char out_buf[32];
    while (ufifo_get(c2, out_buf, sizeof(out_buf)) > 0)
        c2_count++;
    EXPECT_EQ(10, c2_count);
    int c1_count = 0;
    while (ufifo_get(c1, out_buf, sizeof(out_buf)) > 0)
        c1_count++;
    EXPECT_EQ(5, c1_count);
}
...
TEST_F(RecSharedSPMC, Stress)
{
    ...
    for (auto &c : consumers) {
        consumer_threads.emplace_back([&, c]() {
            ...
            while (count < N) {
                char out_buf[32];
                if (ufifo_get(c, out_buf, sizeof(out_buf)) > 0)
                    count++;
                else
                    std::this_thread::yield();
            }
...
TEST_F(RecSharedMPMC, Boundary)
{
    ...
    int consumed = 0;
    char out_buf[32];
    while (ufifo_get(consumer, out_buf, sizeof(out_buf)) > 0)
        consumed++;
    EXPECT_EQ(produced.load(), consumed);
}
...
TEST_F(TagSoleSingle, FifoFullEmpty)
{
    ...
    while (true) {
        ...
        rec->tag = 1;
        memset(rec->data, 'x', 4);
        if (ufifo_put(fifo_, rec, sizeof(TaggedRecord) + rec->size) == 0)
            break;
        count++;
    }
    EXPECT_GT(count, 0);
...
TEST_F(TagSharedSingle, FifoFullEmpty)
{
    ...
    while (true) {
        ...
        rec->size = 4;
        rec->tag = 1;
        if (ufifo_put(fifos_[0], rec, sizeof(TaggedRecord) + rec->size) == 0)
            break;
        count++;
    }
    EXPECT_GT(count, 0);
